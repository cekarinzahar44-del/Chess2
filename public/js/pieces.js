// public/js/pieces.js — robust GLB loader with procedural fallback
...
const PieceFactory = (() => {
  ...
  const whiteMat = new THREE.MeshStandardMaterial({ color: 0xf2e8d0, roughness: 0.22, metalness: 0.06 });
  const blackMat = new THREE.MeshStandardMaterial({ color: 0x1c1008, roughness: 0.28, metalness: 0.14 });

  function createMaterials() { return { whiteMat, blackMat }; }
  ...
  async function _load() {
    try {
      ...
      _extractFromScene(gltf.scene);
      if (Object.keys(templates).length < 4) throw new Error('Too few templates: ' + Object.keys(templates).length);
      _fillMissing();
      console.log('GLB loaded OK:', Object.keys(templates));
    } catch (e) {
      console.warn('GLB failed, using procedural:', e.message);
      _buildProcedural();
    }
  }

  function _extractFromScene(root) {
    const nodes = [];
    root.traverse(n => { if (n.isMesh || n.isGroup) nodes.push(n); });
    console.log('All GLB nodes:', nodes.map(n => n.name));

    nodes.forEach(node => {
      const name = (node.name || '').toLowerCase();
      const type = _typeFromName(name);
      if (!type) return;
      const color = _colorFromNode(node, name);
      const key = `${type}_${color}`;
      if (!templates[key]) {
        templates[key] = _normalizeClone(node);
        console.log(`  Extracted: ${key} from "${node.name}"`);
      }
    });
  }

  function _typeFromName(n) {
  ...
  function _colorFromNode(node, name) {
    if (/black|dark|_b|2$/.test(name)) return 'b';
    if (/white|light|_w|1$/.test(name)) return 'w';
    let brightness = 0.5;
    node.traverse(c => {
      if (c.isMesh && c.material) {
        const m = Array.isArray(c.material) ? c.material[0] : c.material;
        if (m.color) brightness = m.color.r * 0.299 + m.color.g * 0.587 + m.color.b * 0.114;
      }
    });
    return brightness > 0.4 ? 'w' : 'b';
  }

  function _normalizeClone(node) {
    ...
    clone.scale.multiplyScalar(0.85 / maxDim);
  ...
  function _fillMissing() {
    ['k','q','b','n','r','p'].forEach(type => {
      ['w','b'].forEach(color => {
        ...
        const src = templates[other] || templates[Object.keys(templates)[0]];
        if (!src) return;
        const clone = src.clone(true);
        const mat = color === 'w' ? whiteMat : blackMat;
        clone.traverse(c => { if (c.isMesh) c.material = mat; });
        templates[key] = clone;
        console.log(`  Filled missing: ${key}`);
      });
    });
  }
  ...
  function _buildProcedural() {
    ...
    function make(buildFn, mat) {
      const g = new THREE.Group();
      buildFn(geo => { const m=new THREE.Mesh(geo,mat); m.castShadow=m.receiveShadow=true; g.add(m); });
      return g;
    }
    const B = {
  ...
  function createPiece(type, color) {
    const key = `${type}_${color}`;
    if (!templates[key]) { _buildProcedural(); }
